...
- [Klotski! (Pràctica 2 d'AP2, Primavera de 2026)](#klotski-pràctica-2-dap2-primavera-de-2026)
  - [Índex](#índex)
  - [Trencaclosques de peces lliscants](#trencaclosques-de-peces-lliscants)
  - [Objectiu de la pràctica](#objectiu-de-la-pràctica)
  - [Repositori de _puzzles_](#repositori-de-puzzles)
  - [Preparació de l'entorn de treball](#preparació-de-lentorn-de-treball)
    - [WSL (només si esteu a Windows)](#wsl-només-si-esteu-a-windows)
    - [Instal·lació per Linux, MacOS i WSL](#installació-per-linux-macos-i-wsl)
    - [Com executar codi (Paràmetres i Terminal)](#com-executar-codi-paràmetres-i-terminal)
    - [Consells per a Visual Studio Code](#consells-per-a-visual-studio-code)
  - [Fases del projecte](#fases-del-projecte)
    - [1. Descarregar _puzzles_](#1-descarregar-puzzles)
    - [2. Valorar _puzzles_ a partir del seu graf](#2-valorar-puzzles-a-partir-del-seu-graf)
      - [Definició del graf](#definició-del-graf)
      - [Solució del _puzzle_](#solució-del-puzzle)
      - [Mesures al graf](#mesures-al-graf)
      - [Valoració col·laborativa dels _puzzles_](#valoració-collaborativa-dels-puzzles)
    - [3. Generació de _puzzles_ a l'atzar](#3-generació-de-puzzles-a-latzar)
    - [4. Enviament de _puzzles_ nous](#4-enviament-de-puzzles-nous)
  - [Format estàndard d'un _puzzle_ de peces lliscants](#format-estàndard-dun-puzzle-de-peces-lliscants)
    - [Preàmbul: canonicalització de _puzzles_](#preàmbul-canonicalització-de-puzzles)
    - [Format JSON d'un _puzzle_](#format-json-dun-puzzle)
      - [Format dels moviments en un _puzzle_](#format-dels-moviments-en-un-puzzle)
  - [Entrega](#entrega)
  - [Avaluació](#avaluació)
  - [Llista de tasques (per ordre)](#llista-de-tasques-per-ordre)
  - [FAQ](#faq)
...
representar com una seqüència de tripletes `(p, m, d)` on `p` és
l'índex de la peça en el _puzzle_, `m` és un moviment en les 4
possibles direccions: `N`, `E`, `S`, `W` (nord, est, sud i oest),
i `d` és la distància a recórrer, en caselles.En format JSON
aquests moviments seran un array d'arrays de 2 caselles a on la
primera és un enter i la segona un `string` que representa la
direcció de moviment:

```json
[
    [0, "W", 1], [2, "W", 1], [1, "S", 1], [3, "N", 1], [0, "N", 1],
    [3, "E", 1], [0, "N", 1], ...
]
```
...
cost molt baix
([o potser no? 🤔](https://www.wheresyoured.at/the-subprime-ai-crisis-is-here/)),
el
...
- "[Rush Hour](https://www.michaelfogleman.com/rush/)", de
